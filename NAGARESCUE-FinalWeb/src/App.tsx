  ...
  return (
    <div className="text-center mb-8 md:mb-12">
      ...
      <div className="[text-shadow:rgba(0,0,0,0.25)_0px_8px_4px] font-['Poppins',sans-serif] font-bold text-[#0b5ed0] mb-2">
        <p className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl leading-tight">
          NAGA
        </p>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center px-4 sm:px-6 md:px-8 py-8 md:py-12 w-full max-w-lg">
        ...
        <div className="flex flex-col gap-4 sm:gap-6 w-full items-center">
          <button
            ...
            className="w-full max-w-[355px] h-[60px] px-[40px] bg-[#0b5ed0] rounded-[15px] font-['Poppins',sans-serif] font-bold text-white text-lg sm:text-xl md:text-2xl tracking-[4px] sm:tracking-[5.2px] md:tracking-[6.4px] shadow-[0px_2px_8px_0px_rgba(11,94,208,0.3)] hover:bg-[#2f7fb8] transition-colors"
          >
            LOGIN
          </button>
          <button
            onClick={onRegisterClick}
            className="w-full max-w-[355px] h-[60px] px-[40px] border border-[#0b5ed0] border-solid rounded-[15px] font-['Poppins',sans-serif] font-bold text-[#0b5ed0] text-lg sm:text-xl md:text-2xl tracking-[4px] sm:tracking-[5.2px] md:tracking-[6.4px] shadow-[0px_2px_8px_0px_rgba(11,94,208,0.3)] hover:bg-[#0b5ed0] hover:text-white transition-colors"
          >
            REGISTER
          </button>
        </div>
  ...
  if (currentPage === "register") {
    return (
      <Register
        ...
        onNextClick={() => setCurrentPage("rprofile")}
      />
    );
  }

  if (currentPage === "rprofile") {
    return (
      <RProfile
    ...
    return (
      <Profile
        onNavigate={(tab) => {
          ...
          if (tab === "barangay") {
            // Route barangay based on role
            if (userRole === "ComCen Staff") {
              setCurrentPage("deployment");
            } else {
              setCurrentPage("barangayEws");
            }
          }
          if (tab === "saru") {
            // Route SARU based on role
            if (userRole === "ComCen Staff") {
              setCurrentPage("saruComcen");
            } else {
              setCurrentPage("saruMonitoring");
            }
          }
          if (tab === "evacuation")
            setCurrentPage("evacuationMonitoring");
          if (tab === "reports") {
            // Route reports based on role
            if (userRole === "ComCen Staff") {
              setCurrentPage("reportsComcen");
            } else {
        ...
        userRole={userRole === "ComCen Staff" ? "comcen" : "cdrrmo"}
      />
    );
  }
